...
- **执行主体**：OpenAI Codex。
- **执行日期**：2026-07-29。
- **人工部分**：用户审定了检索目标、检索计划和本轮返工边界；没有把聊天中的判断直接当作论文证据。
...
**观察**：没有一个可跨任务套用的“典型一致率”。在最接近本项目的 Re-Centering Humans 中，不同 LLM 判官与人类个性化质量评分的 Spearman 相关约为 `0.111–0.376`；最高值仍不到 `0.4`。`【原文核：附录 G，表 6】`
...
**观察**：Re-Centering Humans 的三名标注员在个性化回答质量上，Spearman 相关为 `0.325`，加权 Cohen’s kappa 为 `0.310`。`【原文核：§6.1】`
...
- 对本项目最准确的风险名称是 **method bias / shared-method variance（方法偏差／共享方法方差）**，并进一步追问**构念效度**。
...
1. **“相关”不是纯文本相似，而是“使用后是否改善这个用户的回答”**。这个目标本身需要用户证据；RUMS 的熵下降是代理量，依赖模型概率与人类效用对齐的假设。`【原文核：RUMS §3.2 的 Interpretation and Limitations】`
2. **人类相关性标签也有主体差异**。Re-Centering Humans 显示，人类内部并非完全一致，模型又倾向过度选择属性。门控器因此不能只追求召回率，还要报告误注入和漏掉有用个性化两类错误。`【原文核：Re-Centering Humans §5.1–§5.3】`
3. **选对记忆不等于用对记忆**。即使检索正确，生成阶段仍可能让旧记忆或偏好压过当前证据；门控和生成时仲裁需要分别测。`【原文核：MemSyco-Bench §4.2、§4.4】`
4. **现有用户价值证据仍薄弱**。RUMS 的主下游比较使用 GPT-4 判官，只有附录中的小规模人类检查；这不足以替代目标用户评价。`【原文核：RUMS §4.4、附录 A.9】`
